...
*/
    ...
    public void fetchExpireMemberCardItemData() throws IOException {
        ...
        if (totalPage > 0) {
            int offSet = 0;
            for (int i = 1; i <= totalPage; i++) {
                response = ConnectionUtil.doGet(StringUtils.replace(MEMBERCARD_URL, "{offset}", String.valueOf(offSet)), ACCEPT, COOKIE, CONNECTION, HOST, ORIGIN, REFERER, USER_AGENT, X_REQUESTED_WITH);
                JsonNode result = MAPPER.readTree(response.returnContent().asString());
                ...
                while (it.hasNext()) {
                    ...
                    String customerId = element.get("CUSTOMERID").asText();
                    String companyName = element.get("SHOPNAME").asText();
                    String cardCode = element.get("CARDNUMBER").asText();
                    String validTime = element.get("MATURITY").asText();
                    String dateCreated = element.get("SORTTIME").asText();//"CREATETIME"
                    ...
                    MemberCard memberCard = new MemberCard();
                    memberCard.setClientId(customerId);
                    memberCard.setCompanyName(companyName);
                    memberCard.setCardCode(cardCode);
        ...
        if (memberCardMap.size() > 0) {
            for (String cardId : memberCardMap.keySet()) {
                MemberCard memberCard = memberCardMap.get(cardId);
                String referer = getMemberCardExpireReferer(cardId, memberCard.getClientId());

                response = ConnectionUtil.doGet(MEMBERCARDDETAIL_URL, ACCEPT, COOKIE, CONNECTION, HOST, ORIGIN, referer, USER_AGENT, X_REQUESTED_WITH);
                JsonNode result = MAPPER.readTree(response.returnContent().asString());
                ...
                while (it.hasNext()) {
                    ...
                    String itemName = element.get("ITEMPRODUCTNAME").asText();
                    String validTime = element.get("STARTTIME").asText();
