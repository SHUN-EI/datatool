    ...
    public void fetchMemberCardDataStandard() throws IOException {
        ...
        if (totalPage > 0) {
            for (int i = 1; i <= totalPage; i++) {
                Response res = ConnectionUtil.doGetWith(MEMBERCARD_URL + i, COOKIE);
                ...
                if (trSize > 0) {
                    for (int j = 1; j <= trSize; j++) {
                        ...
                        String memberCardNameRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(3)";
                        String cardCodeRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(1)";
                        String carNumberRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(2) > div > div";
                        String nameRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(2) > span:nth-child(1)";
                        String phoneRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(2) > span.info";
                        String balanceRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(2) > span:nth-child(4)";
                        String dateCreatedRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(8)";
                        String validTimeRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(5)";
                        String remarkRegEx = trRegEx + ":nth-child(" + j + ") > td:nth-child(7)";

                        String memberCardName = body.select(memberCardNameRegEx).text();
                        String cardCode = body.select(cardCodeRegEx).text();
                        ...
                        String dateCreated = body.select(dateCreatedRegEx).text();
                        String validTime = body.select(validTimeRegEx).text();
                        String remark = body.select(remarkRegEx).text();
                        ...
                        Response resp = ConnectionUtil.doGetWith(MEMBERCARDITEM_URL + id, COOKIE);
                        String content = resp.returnContent().asString();
                        Document doc = Jsoup.parseBodyFragment(content);


                        String serviceTrRegEx = "#tab_services > tbody > tr";
                        int serviceTrSize = WebClientUtil.getTagSize(doc, serviceTrRegEx, HtmlTag.trName);
        ...
        String pathname = "C:\\exportExcel\\惠车邦会员卡.xls";
